...
cloudinary.config({
  cloud_name: "dazzy4wzq",
  api_key: "533172113942665",
  api_secret: "uVpQS63ksN6gF-yT9OuUf_BN3mI", // Click 'View API Keys' above to copy your API secret
});
...
export const createProduct = async (req, res) => {
  try {
    if (req.user.role !== "admin") {
      return res
        .status(403)
        .json({ message: "Acción no permitida. Solo para administradores." });
    }

    const { name, description, price, stock, category, marca } = req.body;
    ...
    return res
      .status(500)
      .json({ message: "Hubo un fallo al crear el producto" });
  }
};
...
export const addReview = async (req, res) => {
  try {
    ...
    if (req.user.role !== "cliente") {
      return res
        .status(403)
        .json({ message: "Acción no permitida. Solo para clientes." });
    }
